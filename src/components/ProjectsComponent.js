            ...
            >
                ...
                <div className='w-full h-20 flex flex-row justify-center items-center mt-6'>
                    {project.images.map(image =>
                        <img key={image.toString()} src={image.toString()} alt=''
                             className='max-w-16 max-h-16 rounded-sm mx-2 outline outline-white-secondary transition-all hover:cursor-pointer hover:scale-105'
...
const ProjectComponent = ({project, selectCallback}) => (
    <div
        className='w-96 h-96 text-white-primary rounded-sm bg-secondary-faded flex flex-col m-4 shadow transform transition-all hover:scale-110 hover:cursor-pointer'
        onClick={() => selectCallback(project)}>
        <div
            className='w-full h-40 rounded-tl-sm rounded-tr-sm w-full bg-white-primary bg-opacity-40 bg-contain bg-center bg-no-repeat bg-primary border-white-secondary border-b-4'
            style={{backgroundImage: `url(${project.logo})`}}/>

        <div className='w-full h-56 p-4 rounded-bl-sm rounded-br-sm'>
            <span
                className='text-xl font-bold transition-colors duration-300 hover:text-link-hover'>{project.name}</span>
